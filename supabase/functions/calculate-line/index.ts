...
Deno.serve(async (req) => {
  ...
  // RLS limits this read to sets belonging to the authenticated user's sessions.
  ...
  // RLS also scopes LINE history to this user.
  ...
  if (previous) {
    ...
    // A streak means three separate training sessions, not three sets from one
    // workout. Use the best set in each of the three most recent sessions.
    ...
    for (const s of sets!) {
      if (!recentSessionIds.includes(s.session_id)) {
        recentSessionIds.push(s.session_id);
      }
  ...
  // Competitive state is server-authored. user.id comes from the verified JWT,
